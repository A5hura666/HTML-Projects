...
async function displayhistory(url) {
    let req = await fetch(`https://gigondas.iut-valence.fr:1112/sprietna/ihm/tp4/users/history/${url}`);
    ...
    for (let element = 0; element < 50/*schedules.length*/; element++) {
        let hours = parseInt(Math.floor(schedules[element].schedule.travel.duration / 60));
        let minutes = parseInt(schedules[element].schedule.travel.duration % 60);
        let departhours = parseInt(((schedules[element].schedule.departureTime).split(':'))[0])
        let departminutes = parseInt(((schedules[element].schedule.departureTime).split(':'))[1])
		...
		<article class="ticket ticket_n${element}">
								...
								<div class="traveldetail">
									<section class="trajet depart">
										...
										<div>
											...
											<span class="start">${schedules[element].schedule.travel.from.city} - ${schedules[element].schedule.departureTime}</span>
										</div>
									</section>
									<p>→</p>
									<section class="trajet arrivée">
										...
										<div>
											...
											<span class="start">${schedules[element].schedule.travel.to.city} - ${arrivalhours + "h" + arrivalminutes}</span>
										</div>
									</section>
								</div>
								<div class="priceANDbook">
									<article id="price">
										<p class="price">${schedules[element].schedule.price}€</p>
									</article>
							...
							<section class="bottomticket">
								<div class="date">
									...
									<span>${schedules[element].schedule.date}</span> 
								</div>
								<div class="duration">
								...
								<div class="price">
									...
									<span>${schedules[element].schedule.price}€</span>
								</div>
							</section>
						</article>
		`
