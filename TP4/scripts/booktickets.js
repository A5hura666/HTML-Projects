croziflette();
function croziflette(){
    let cookie = document.cookie.split(";")
    for (let i = 0; i < cookie.length; i++) {
        
        if (String(cookie[i]).split('=')[0].includes('BuyTicket')) {
            let ticketsid= String(cookie[i]).split('=')[1].split(',')
            for(let j=0;j<ticketsid.length;j++){
                dispalycart(ticketsid[i]);
...
async function dispalycart(element) {
    let request = await fetch(`https://gigondas.iut-valence.fr:1112/sprietna/ihm/tp4/schedules/info/${element}`);
    let req = await request.json();


    let hours = parseInt(Math.floor(req.travel.duration / 60));
    let minutes = parseInt(req.travel.duration % 60);
    let departhours = parseInt(((req.departureTime).split(':'))[0])
    let departminutes = parseInt(((req.departureTime).split(':'))[1])
    ...
    document.querySelector('.mainto > section').innerHTML += `
                <article class="ticket ticket_n${element}">
                                    <section class="topticket">
                                        <div class="traveldetail">
                                            <section class="trajet depart">
                                                ...
                                                <div>
                                                    ...
                                                    <span class="start">${req.travel.from.city} - ${req.departureTime}</span>
                                                </div>
                                            </section>
                                            <p>→</p>
                                            <section class="trajet arrivée">
                                                ...
                                                <div>
                                                    ...
                                                    <span class="start">${req.travel.to.city} - ${arrivalhours + "h" + arrivalminutes}</span>
                                                </div>
                                            </section>
                                        </div>
                                        <div class="priceANDbook">
                                            ...
                                            <button onclick="bookticket(${req.id})" class="tickket ${req.id}">Réserver</button>
                                        </div>
                                    </section>
